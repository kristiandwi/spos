	<!-- Footer start -->
	<div class="ts-footer">
		<div class="container">
			<div class="row ts-gutter-30 justify-content-lg-between justify-content-center">
				<div class="col-lg-7 col-md-7">
					<div class="footer-widget">
						<div class="footer-logo">
							<!--img src="https://www.solopos.com/assets/13th-solopos.png" alt="Footer Logo"-->
							<img src="<?php echo ASSETS_PILKADA; ?>images/logo-solopos.png" alt="Footer Logo">
						</div>
						<div class="widget-content">
							<ul class="ts-footer-nav">
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">KONTAK</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">DISCLAIMER</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">FORM PENGADUAN</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">REDAKSI</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">TENTANG KAMI</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">PEDOMAN MEDIA SIBER</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">KARIR</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">KODE ETIK</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">METODE CEK FAKTA</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">SITEMAP</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">HAK JAWAB DAN KOREKSI BERITA</a></li>

							</ul>
						</div>
					</div>
				</div><!-- col end -->
				<div class="col-lg-5 col-md-5">
					<div class="footer-widtet post-widget">
						<h3 class="widget-title"><span>Jaringan</span></h3>
						<div class="widget-content">

							<ul class="ts-footer-nav">
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">BISNIS.COM</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">RUMAH190.COM</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">GRIYA190.COM</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">IKLAN</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">SEMARANGPOS.COM</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">MADIUNPOS.COM</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">JEDA.ID</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">HARIANJOGJA.COM</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">PASANGBALIHO.COM</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">TOKO</a></li>
								<li class="ts-footer-nav-item"><a class="ts-footer-nav-item-link" href="">IBUKOTAKITA.COM</a></li>

							</ul>							

						</div>
					</div>
				</div><!-- col end -->
			</div><!-- row end -->
		</div><!-- container end -->
	</div>
	<!-- Footer End-->

	<!-- ts-copyright start -->
	<div class="ts-copyright">
		<div class="container">
			<div class="row align-items-center justify-content-between">
				<div class="col-12 text-center">
					<div class="copyright">
						<p>Copyright &copy; 2020, Solopos Digital Media - Creative Information Network. All rights reserved.</p>
					</div>
				</div>
			</div>
		</div>
	</div>
	<!-- ts-copyright end-->

	<!-- backto -->
	<div class="top-up-btn">
		<div class="backto" style="display: block;"> 
			<a href="#" class="icon icon-arrow-up" aria-hidden="true"></a>
		</div>
	</div>
	<!-- backto end-->
	
	<!-- Javascript Files
	================================================== -->

	<!-- initialize jQuery Library 
	<script src="<?php echo ASSETS_PILKADA; ?>js/jquery.js"></script>-->
	<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/2.1.0/jquery.min.js" integrity="sha512-qp27nuUylUgwBZJHsmm3W7klwuM5gke4prTvPok3X5zi50y3Mo8cgpeXegWWrdfuXyF2UdLWK/WCb5Mv7CKHcA==" crossorigin="anonymous"></script>
	<!-- Popper Jquery -->
	<script src="<?php echo ASSETS_PILKADA; ?>js/popper.min.js"></script>
	<!-- Bootstrap jQuery -->
	<script src="<?php echo ASSETS_PILKADA; ?>js/bootstrap.min.js"></script>
	<!-- magnific-popup -->
	<script src="<?php echo ASSETS_PILKADA; ?>js/jquery.magnific-popup.min.js"></script>
	<!-- Owl Carousel -->
	<script src="<?php echo ASSETS_PILKADA; ?>js/owl.carousel.min.js"></script>
	<!-- Color box -->
	<script src="<?php echo ASSETS_PILKADA; ?>js/jquery.colorbox.js"></script>
	<!-- Template custom -->
	<script src="<?php echo ASSETS_PILKADA; ?>js/custom.js"></script>
	<?php wp_footer(); ?>
</body>
</html>