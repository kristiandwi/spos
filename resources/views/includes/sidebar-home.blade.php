				...
				<ul class="list-post">
		          ...
		          @foreach($bisnis as $bs) @if($bs_loop <= 3)	
					<li>
						<div class="post-block-style media">
							<div class="post-thumb">
								<a href="{{ url("/{$bs['slug']}-{$bs['id']}") }}?utm_source=sidebar_desktop" title="{{ $bs['title'] }}">
									<img class="img-fluid" src="{{ $bs['images']['thumbnail'] }}" alt="{{ $bs['title'] }}" style="object-fit: cover; object-position: center; height: 85px; width: 85px;">
								</a>
							</div><!-- Post thumb end -->
